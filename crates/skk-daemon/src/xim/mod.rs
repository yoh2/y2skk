...
/// in-process.  Returns a `JoinHandle` that resolves when the X connection
/// drops or a fatal error occurs.
pub fn spawn(sessions: Arc<Mutex<SessionManager>>) -> tokio::task::JoinHandle<Result<()>> {
    tokio::task::spawn_blocking(move || run(sessions))
}

/// Blocking XIM server entry point.
fn run(sessions: Arc<Mutex<SessionManager>>) -> Result<()> {
    ...
    let keymap = KeyMap::new(conn.as_ref(), conn.setup()).context("failed to fetch keyboard mapping")?;
    ...
    let mut handler = Handler::new(sessions, keymap, preedit);
