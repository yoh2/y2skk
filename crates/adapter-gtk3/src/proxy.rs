...
///
/// Uses `zbus::blocking` so it can be called from synchronous GTK3 callbacks
/// without requiring an async runtime in the adapter process.
...
impl DaemonProxy {
    ...
    /// Connects to the session bus and verifies the daemon is reachable.
